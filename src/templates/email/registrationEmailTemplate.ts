...
export const registrationEmailTemplate = (code: string): string => {
   const template: string = `
      ...
      <p ${commonStyles.p}>Aktywuj konto klikając <a href="https://${frontendData.accountActivationPaths.editor}/${code}" target="_blank">tutaj</a>.</p>
      ...
      <table ${commonStyles.contentTable}>
         <tr ${commonStyles.contentTr}>
            <td ${commonStyles.contentTd}>
               <a href="https://${frontendData.accountActivationPaths.editor}/${code}" target="_blank">https://${frontendData.accountActivationPaths.editor}/${code}</a>
            </td>
         </tr>
      </table>
      <p ${commonStyles.p}>Link wygasa po 24 godzinach. Jeśli nie jesteś odbiorcą wiadomości możesz ją zignorować.</p>
   `;
